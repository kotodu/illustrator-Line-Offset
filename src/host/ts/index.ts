 ...
 * @param {number} x4 
 * @param {number} y4 
 * @return {Array<number>} [x,y] 交点のPathItem
 */
function crossPoint(
    x1: number,
    y1: number,
 ...
 */
function joinAndCheckPoints(optionsP1: any[],optionsP2: any[],mTrue: number): Array<Array<number>>{
 ...
 * mはその線分の傾き
 * @param {Array} points 
 * @return {Array} baseSegments [線分データ1,線分データ2,線分データ3...]
 */
function createBaseSegments(points:Array<any>): Array<any> {
    var baseSegments = [[]];
 ...
 * @returns {Array<Array<Number>>} result [左のPathPoints配列,右のPathPoints配列]
 */
 ...
 * @param {Array<Array<Number>>} points points、[[x1,y1],[x2,y2]...]
 * @param {Number} offsetWidth オフセット幅(px)。正の数も負の数も来る
 */
function createOffsetPath(points,offsetWidth:number){
